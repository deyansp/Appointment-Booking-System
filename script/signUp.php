...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (empty($_POST["firstName"]) || empty($_POST["lastName"]) || empty($_POST["newEmail"]) || empty($_POST["newPsw"])) {
        ...
        $statement = $connection->prepare("SELECT StudentEmail FROM discoveryUniUsers WHERE StudentEmail = ?");
        ...
        if ($row) { 
            ...
            $statement = $connection->prepare("INSERT INTO discoveryUniUsers (FirstName, LastName, StudentEmail, Password) VALUES (?, ?, ?, ?)");
            ...
            header("location: /~1704796/CMP206/index.html");
        }
    }
}
?>